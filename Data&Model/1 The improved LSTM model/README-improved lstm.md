...
│
└── README-improved lstm.md                 # Project documentation
...
test_metrics.csv: stores key evaluation metrics（Main experimental results *Table7-Improved LSTM and Table5 Fusion method III (Improved LSTM)）
...
results/backtest_results.csv(*Table8-Improved LSTM)
...
```bash
python All_inference.py
```
All ablation experiment results can be reproduced
...
ablation experiment/summary_results(*fig6)

- **Detailed prediction results (up/down)** are stored in: 
ablation experiment/summary_results/all_results_summary.xlsx Accuracy Summary sheet(*Table9) 

```bash
cd "LSTM (without attention mechanism)"
python test.py
```
Output：

- **Detailed prediction results (up/down)** are stored in: 
ablation experiment/LSTM (without attention mechanism)/test_results/accuracy_summary.xlsx(*Table9-LSTM (without attention mechanism)) 

## Different Fusion Methods/

This section enables reproduction of the results for Fusion Method 1 and Fusion Method 2.

The test_all.py script located in each fusion method directory performs batch evaluation across five temporal segments. 

It automatically iterates over the corresponding model checkpoints, partitions the test set according to each segment's data range, runs inference, and consolidates all evaluation metrics into a unified comparison table.
...
python test_all.py.
```
Upon completion of testing, summary.csv is generated under the test_results_all/ directory(*Table5-Fusion method I and II).
