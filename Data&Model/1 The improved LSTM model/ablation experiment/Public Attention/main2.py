...
def train(dataset_loader, net, loss_function, optimizer):
    ...
    for btsz, (shuju_1, shuju_2, shuju_3, shuju_4, label) in enumerate(dataset_loader):
        ...
        # n_shuju_1 = shuju_2.permute(2, 0, 1).float().to(device)
        # n_shuju_1 = shuju_3.permute(2, 0, 1).float().to(device)
        n_shuju_1 = shuju_4.permute(2, 0, 1).float().to(device)
...
def test(full_data, net, loss_function, device):
    ...
    # shuju_1 = torch.tensor(full_data['shuju_2']).permute(2, 0, 1).float().to(device)
    # shuju_1 = torch.tensor(full_data['shuju_3']).permute(2, 0, 1).float().to(device)
    shuju_1 = torch.tensor(full_data['shuju_4']).permute(2, 0, 1).float().to(device)
