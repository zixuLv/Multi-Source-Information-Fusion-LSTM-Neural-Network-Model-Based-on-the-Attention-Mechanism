...
# Target accuracy to save checkpoint
TARGET_ACC = 0.6165
ACC_TOL    = 1e-4   # tolerance: saves when |acc - TARGET_ACC| < ACC_TOL
...
if __name__ == "__main__":
    ...
    no_improvement = 0
    patience       = 500
    target_saved   = False   # flag: stop once target accuracy is saved
    ...
    for epoch in range(C.epoch_zong):
        ...
        if (epoch + 1) % C.epoch_show == 0:
            ...
            # ── Save checkpoint when accuracy matches target ─────
            if abs(test_acc - TARGET_ACC) < ACC_TOL and not target_saved:
                target_ckpt = f"ckpt/target_{TARGET_ACC}_epoch{epoch + 1}.pth"
                torch.save(net.state_dict(), target_ckpt)
                print(f"Target accuracy {TARGET_ACC} reached! Checkpoint saved: {target_ckpt}")
                # target_saved = True   # save only the first match; remove this line to save all matches
            # ────────────────────────────────────────────────────
